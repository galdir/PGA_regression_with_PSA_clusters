...
def build_dnn_model(n_hidden_layers: int = 3, n_neurons: int = 419, 
                    activation: str = "swish", learning_rate: float = 0.0005656677780014923) -> Sequential:
    """
...
def tune_random_forest(X_train: pd.DataFrame, y_train: pd.Series, preprocessor: ColumnTransformer, 
                       n_trials: int = 300, random_state: int = 42):
    """
    Executa a busca de hiperparâmetros para o Random Forest usando Optuna.
    """
    def objective(trial):
        param = {
...
def tune_dnn(X_train, y_train, X_valid, y_valid, preprocessor: ColumnTransformer, 
    ...
    tuner.search(tf_x_train, y_train, validation_data=(tf_x_valid, y_valid), 
